...
export default {
      ...
      this.$msgbox
        ...
        .then(() => {
          let returndata = {
            ...
            // institutionname: '文化大会',
            institutionname: this.form.institution_name,
            adminitorid: '',
            isdone: '待审核',
            audittime: '',
          }
          console.log("提交的数据",returndata);
          console.log('是重新提交',reput);
          if (reput) {
            // 是重新提交 则用put
            // 是初次提交 则用post
            ...
            // 是初次提交 则用post
              ...
              .then((res) => {
                this.uploadPhoto(returndata.workerid,returndata.title);
                console.log("资质申请提交成功！",res);
                this.$message({
                  type: 'success',
                  message: '资质申请提交成功！'
                });
  ...
  mounted: function () {
    ...
    if (this.form.role == '医生') {
      // 获取医生信息
      this.$http
        .get('/API/Doctor/id?doctorid='+this.form.user_id,{
          headers: {
            TokenValue: this.access_token,
          },
        })
        .then((res)=>{
            ...
            console.log("获取的社区号",communityid);
            this.$http  
              .get('/API/community/id?id='+communityid,{
                headers: {
                  TokenValue: this.access_token,
                },
              })
              .then((res)=>{
                ...
                this.form.institution_name = communityname.NAME;
                console.log("医生的社区信息获取成功！",communityname);
              })
              .catch((err)=>{
                console.log("社区获取失败",err);
                // AccessTokenFailed();
              })
        })
        .catch((err)=>{
